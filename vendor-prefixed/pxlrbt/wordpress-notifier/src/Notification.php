...
namespace pxlrbt\CF7WeClapp\Vendor\pxlrbt\WordpressNotifier;

use InvalidArgumentException;
use pxlrbt\CF7WeClapp\Vendor\pxlrbt\WordpressNotifier\Contracts\Notification as NotificationContract;
/**
...
class Notification implements NotificationContract
{
    ...
    {
        \printf('<div class="notice notice-%s%s%s" id="%s"><p>%s%s</p></div>', $this->type, $this->isDismissible() ? ' is-dismissible' : '', $this->classes ? ' ' . $this->classes : '', $this->id, !empty($this->title) ? '<strong>' . $this->title . '</strong>' : '', $this->message);
    }
}
